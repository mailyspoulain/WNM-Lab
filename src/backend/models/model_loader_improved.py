...
def load_chest_xray_model(model_path=None):
    ...
    """
    try:
        # Obtenir le chemin absolu du dossier où se trouve ce fichier
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Chemins possibles relatifs au dossier du module
        possible_paths = [
            os.path.join(current_dir, 'densenet121_chest_xray_model.pth'),
            os.path.join(current_dir, 'densenet121_chest_xray_best.pth'),
            os.path.join(current_dir, 'chest_xray_model.pth'),
            os.path.join(current_dir, 'vgg16_chest_xray_model.pth'),
            # Chemins legacy
            'backend/models/densenet121_chest_xray_model.pth',
            'backend/models/densenet121_chest_xray_best.pth',
            'backend/models/chest_xray_model.pth',
            'src/backend/models/densenet121_chest_xray_model.pth',
            'src/backend/models/densenet121_chest_xray_best.pth',
        ]
        ...
        if not selected_path:
            logger.error(f"Aucun modèle trouvé. Chemins testés : {possible_paths[:5]}...")
            return None, None, None

        logger.info(f"Chargement du modèle depuis : {selected_path}")
        
        # Charger avec weights_only=False pour PyTorch 2.6+
        checkpoint = torch.load(selected_path, map_location='cpu', weights_only=False)
        
        # ----- DENSENET -----
        if "densenet" in selected_path.lower():
            model = models.densenet121(weights=None)
            # IMPORTANT : répliquer *strictement* la même architecture du classifier
        ...
        # ----- VGG16 -------
        elif "vgg16" in selected_path.lower():
        ...
        logger.info(f"✅ Modèle chargé avec succès depuis {selected_path}")
...
def predict_with_tta(model, image, transform, n_augmentations=5, temperature=1.0):
    """
    ...
    with torch.no_grad():
        ...
        output = model(tensor)
        probs = torch.softmax(output / temperature, dim=1).cpu().numpy()
...
def predict_pneumonia_improved(model_info, image_path, 
    ...
    try:
        ...
        if use_tta and model is not None:
            ...
            probs, stds = predict_with_tta(model, image, transform, temperature=temperature)
